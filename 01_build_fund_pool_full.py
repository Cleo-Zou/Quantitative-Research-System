"""
01  基金池构建 —— 全市场扫描版（Discovery Mode）

用途: 首次构建基金池时使用，从全市场 10000+ 基金中筛选指数增强基金。
流程:
  1. 获取全市场基金列表（akshare 天天基金）
  2. 粗筛: 名称关键词匹配 → 候选池
  3. 精细验证: API 逐一确认基准指数、增强策略
  4. 输出初筛 Excel → 人工复核 → 整理为正式白名单 Excel

日常维护请使用 01_build_fund_pool1.py（白名单维护版）。
"""
