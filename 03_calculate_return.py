...
def _fetch_index_history(index_code: str) -> pd.DataFrame | None:
    ...
    if index_code in em_secids:
        result = _fetch_index_from_eastmoney(em_secids[index_code], index_code)
        if result is not None:
            return result
        print(f"  ⚠ 东方财富全失败，回退 AKShare...")
...
def _fetch_index_from_eastmoney(secid: str, index_code: str) -> pd.DataFrame | None:
    """直连东方财富 HTTP API 拉取指数日线（一次请求全量，绕过 AKShare 分页）。"""
    ...
    for attempt in range(10):
        try:
            req = urllib.request.Request(url, headers={
                ...
                "Referer": f"https://quote.eastmoney.com/zs{secid.split('.')[1]}.html",
            })
            ...
            result = result.dropna(subset=["date", "index_value"])
            result = result.sort_values("date").reset_index(drop=True)
            print(f"  ✓ {label}(东方财富): {len(result)} 条, 最新 {result['date'].max()}")
            return result
        except Exception as e:
            if attempt < 9:
                wait = 3 + attempt * 2
                print(f"  [WARN] {label} 第{attempt+1}次失败: {e}，{wait}s 后重试...")
                time.sleep(wait)
            else:
                print(f"  [WARN] {label} 10次全失败")
